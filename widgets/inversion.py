    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            if imgui.collapsing_header("Webcam"):
                ...
                immvision.image_display_resizable(f"webcam_image", frame, ImVec2(frame.shape[1], frame.shape[0]), refresh_image=True, is_bgr_or_bgra=False)
            ...
            for i, image in enumerate(self.loaded_images):
                ...
                immvision.image_display_resizable(f"image_{i}", image, ImVec2(im_size, int(im_size / aspect_ratio)), refresh_image=False, is_bgr_or_bgra=False)
                imgui.same_line(spacing=0)

            imgui.new_line()
            imgui.text("\n2. Image Preprocessing")
            if imgui.button("Preprocess", ImVec2(viz.label_w_large, 0)):
                viz.args.inversion_images = np.array(self.files_to_preprocess)
                self.files_to_preprocess = []

            for i, image in enumerate(viz.preprocessed_images):
                immvision.image_display_resizable(f"image_pre_{i}", np.ascontiguousarray(image), ImVec2(im_size, im_size), refresh_image=False, is_bgr_or_bgra=False)
                imgui.same_line(spacing=0)
