...
def load_cond(num):
    cond = torch.zeros([num, 1 + 30 + 30 + 30], device="cuda", dtype=torch.float)
    ...
    for i, key in enumerate(keys):
        for offset, attribute in enumerate(attributes):
            if key in annotations[attribute].keys():
                cond[i, 1 + 30 * offset: 1 + 30 * (offset + 1)] = torch.tensor(annotations[attribute][key], device="cuda", dtype=torch.float)
    return cond
...
class LatentMapRandom:
    ...
    def load_w_map(self, mapping_network, template_mapping=None, z_map=None, truncation_psi=1.0):
        if z_map is not None:
            self.z_map = z_map
        ...
        self.w_map = mapping_network(reshaped_z_map, c=template_mapping(cond), truncation_psi=truncation_psi)
